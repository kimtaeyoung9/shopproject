...
import com.shopproject.dto.OrderHistDto;
import com.shopproject.dto.OrderItemDto;
import com.shopproject.entity.*;
...
public class OrderService {
    ...
    public Page<OrderHistDto> getOrderList(String email, Pageable pageable){
        ...
        Long totalCount = orderRepository.countOrder(email);//유저의 주문 총 개수를 구합니다.
        List<OrderHistDto> orderHistDtos = new ArrayList<>();

        for(Order order : orders){//주문 리스트를 순회하면서 구매 이력 페이지에 전달할 DTO 를 생성합니다.
        ...
        return new PageImpl<OrderHistDto>(orderHistDtos, pageable, totalCount);//페이지 구현 객체를 생성하여 반환합니다.
    }
    @Transactional(readOnly = true)
    public boolean validateOrder(Long orderId, String email){//현재 로그인한 사용자와 주문 데이털르 생성한 사용자가 같은지 검사를 합니다.
    ...
    public void cancelOrder(Long orderId){
        Order order = orderRepository.findById(orderId)
                .orElseThrow(EntityNotFoundException::new);
        order.cancelOrder();//주문 취소 상태로 변경하면 변경 감지 기능에 의해서 트랜잭션이 끝날 떄 update 쿼리가 실행됩니다.
    }
    public Long orders(List<OrderDto> orderDtoList, String email){
        ...
        Order order = Order.createOrder(member, orderItemList);//현재 로그인 회원 주문 상품 목록 주문 엔티티 생성
